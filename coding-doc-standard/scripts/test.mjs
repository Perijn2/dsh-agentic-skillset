...
const CHECKER = fileURLToPath(new URL("../src/checker.py", import.meta.url))
const tmp = mkdtempSync(join(tmpdir(), "coding-doc-"))
const PYTHON_COMMAND = process.platform === "win32" ? ["py", "-3"] : ["python3"]
...
function check(name, content, ext, expected, configPath) {
  ...
  try {
    execFileSync(PYTHON_COMMAND[0], [...PYTHON_COMMAND.slice(1), ...args], { stdio: "pipe" })
    const code = 0
...
function checkCandidate(name, path, content, expected, configPath) {
  try {
    ...
    if (configPath) args.push(configPath)
    args.push("--stdin")
    execFileSync(PYTHON_COMMAND[0], [...PYTHON_COMMAND.slice(1), ...args], {
      input: content,
      stdio: "pipe",
    })
...
function checkHook(name, payload, expected) {
  const wrapper = fileURLToPath(new URL("../run-check.sh", import.meta.url))
  try {
    execFileSync("bash", ["-c", 'printf %s "$1" | bash "./run-check.sh"',
      "hook-test", JSON.stringify(payload)], {
      cwd: dirname(wrapper),
      stdio: "pipe",
    })
    _report(name, 0, expected)
  } catch (error) {
    if (name === "hook: allows compliant write") console.error(error.stderr?.toString())
    _report(name, error.status ?? 1, expected)
  }
}
...
// (8) the installer must preserve only reference filenames when targeting the
// machine-wide skill directory. This catches Windows paths being embedded in
// the target path instead of copied beneath the skill directory.
try {
  const packageRoot = fileURLToPath(new URL("..", import.meta.url))
  const output = execFileSync("node", ["scripts/install.mjs", "--dry-run"], {
    cwd: packageRoot,
    encoding: "utf8",
  })
  const duplicatedSourcePath = output.includes(`coding-doc-standard\\${packageRoot}`)
  const listsReferenceFilename = /would install reference .*coding-doc-standard[\\/]c-cpp\.md/.test(output)
  _report("installer: references are placed directly in the skill directory",
    duplicatedSourcePath || !listsReferenceFilename ? 1 : 0, 0)
} catch (error) {
  _report("installer: references are placed directly in the skill directory", error.status ?? 1, 0)
}
